require_relative "../firefly"

describe Firefly do
  let(:mascot) { Firefly.new() }

  it "is bug-type" do
    expect(mascot.type).to eq :bug
  end

  it 'has normal status' do
    expect(mascot.status).to eq :normal
  end

  describe "base stats" do
    it "has a base hit point value" do
      expect(mascot.base_hit_point_value).to eq 60
    end

    it "has a base attack strength" do
      expect(mascot.base_attack_strength).to eq 45
    end

    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 50
    end
  end

  describe "experience and leveling up" do
  ...
  describe "attacking and defending" do
    ...
    it "is resistant to flying type attacks" do
      expect(mascot.resistant_to?(:flying)).to be true
    end
    ...
    it "is susceptible to water type attacks" do
      expect(mascot.susceptible_to?(:water)).to be true
    end

    it "is not susceptible to other types of attacks" do
      expect(mascot.susceptible_to?(:flying)).to be false
    end

    it "is immune to ground type attacks" do
      expect(mascot.immune_to?(:ground)).to be true
    end
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:bug_bite, :buzz, :signal_beam, :scissor]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:buzz)).to be true
      end
