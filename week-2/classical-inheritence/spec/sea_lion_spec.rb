require_relative "../sea_lion"

describe SeaLion do
  let(:mascot) { SeaLion.new() }

  it "is water-type" do
    expect(mascot.type).to eq :water
  end
  ...
  describe "base stats" do
    it "has a base hit point value" do
      expect(mascot.base_hit_point_value).to eq 65
    end
    ...
    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 55
    end
  end
  ...
  describe "attacking and defending" do
    ...
    it "is immune to flying type attacks" do
      expect(mascot.immune_to?(:flying)).to be true
    end

    it "is not immune to other types of attacks" do
      expect(mascot.immune_to?(:ground)).to be false
    end

    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:water_pulse, :bubble_beam, :surf, :body_slam]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:surf)).to be true
      end
