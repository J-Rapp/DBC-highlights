...
describe Otter do
  ...
  describe "base stats" do
    ...
    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 70
    end
  end
  ...
  describe "attacking and defending" do
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:water_pulse, :bubble_beam, :aqua_tail, :water_spout]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:aqua_tail)).to be true
      end
