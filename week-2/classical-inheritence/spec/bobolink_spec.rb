...
describe Bobolink do
  let(:mascot) { Bobolink.new() }

  it "is flying-type" do
    expect(mascot.type).to eq :flying
  end
  ...
  describe "base stats" do
    it "has a base hit point value" do
      expect(mascot.base_hit_point_value).to eq 40
    end

    it "has a base attack strength" do
      expect(mascot.base_attack_strength).to eq 60
    end

    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 30
    end
  end
  ...
  describe "attacking and defending" do
    ...
    it "is not susceptible to other types of attacks" do
      expect(mascot.susceptible_to?(:ground)).to be false
    end
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:wing_attack, :air_slash, :fury_attack, :razor_wind]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:fury_attack)).to be true
      end
