...
describe Nighthawk do
  let(:mascot) { Nighthawk.new() }
  ...
  describe "base stats" do
    ...
    it "has a base defense strength" do
      expect(mascot.base_defense_strength).to eq 40
    end
  end
  ...
  describe "attacking and defending" do
    ...
    describe "attacks" do
      it "knows attacks" do
        expect(mascot.attacks).to match_array [:wing_attack, :air_slash, :peck, :hurricane]
      end

      it "reports when an attack is known" do
        expect(mascot.knows_attack?(:peck)).to be true
      end
